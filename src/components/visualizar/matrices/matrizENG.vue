...
<mdb-container class="my-4">
    ...
    <mdb-card class="mb-2" v-show="editar">
        <mdb-card-body>
            ...
            <hr/>
            <mdb-card-title>Elastic modulus (kN/m<sup>2</sup>)</mdb-card-title>
            <hr/>
            <mdb-row>
            ...
            <hr/>
            <mdb-card-title>Dimensions of cross-sections (m)</mdb-card-title>
            <small class="text-muted">The section width indicates the dimension perpendicular 
                to the plane of the structure, i.e. the dimension parallel to the bending axis.</small>
            <hr/>
            <mdb-row v-for="(seccion, i) in datos.secciones" :key="'s'+i">
                <mdb-col md="6" col="md">
                    <p class="text-center"><b>Width of section {{i+1}}</b></p>
                    <mdb-input :class="seccion[1].min === seccion[1].max ? 'disabled' : ''" type="number" :label="'Width '+seccion[0]+' (m)'"
                ...
                <mdb-col md="6" col="md">
                    <p class="text-center"><b>Height of section {{i+1}}</b></p>
                    <mdb-input :class="seccion[2].min === seccion[2].max ? 'disabled' : ''" type="number" :label="'Height '+seccion[0]+' (m)'"
            ...
            <hr/>
            <mdb-card-title>Coordinates of the nodes</mdb-card-title>
            <hr/>
            <mdb-row v-for="(nodo, i) in datos.nodos" :key="'n'+i" class="my-3">
                <mdb-col md="6" col="md">
                    <p class="text-center"><b>X coordinate of the node {{i+1}} (m)</b></p>
                    <mdb-input :class="nodo[1].min === nodo[1].max ? 'disabled' : ''" type="number" :label="'X coordinate '+nodo[0]"
                ...
                <mdb-col md="6" col="md">
                    <p class="text-center"><b>Y coordinate of the node {{i+1}} (m)</b></p>
                    <mdb-input :class="nodo[2].min === nodo[2].max ? 'disabled' : ''" type="number" :label="'Y coordinate '+nodo[0]"
            ...
            <hr/>
            <mdb-card-title>Boundary conditions</mdb-card-title>
            <small class="text-muted">Mark the blocked degrees of freedoms in each node:</small>
            <hr/>
            <div v-for="(nodo, i) in datos.bc" :key="'bc'+i">
                <span class="mt-3 mx-4"><b>Node <sub>{{i+1}}</sub></b></span>
            ...
            <hr/>
            <mdb-card-title>Initial and final nodes of each beam</mdb-card-title>
            <hr/>
                        ...
                        <tr v-for="(barra, i) in datos.barras" :key="'b'+i">
                            ...
                            <td>
                                <select class="browser-default custom-select my-2" v-model="barra[4]">
                                    <option selected :value="undefined" disabled>Section</option>
                                    <option v-for="(seccion, i) in datos.secciones" :key="'saux'+i" :value="i+1">Section {{i+1}}</option>
                                </select>
                            </td>
                            <td>
                                <mdb-btn block color="purple" @click="cambiaBarras(i)"><mdb-icon icon="exchange-alt" class="mr-1"/> Swap</mdb-btn>
                            </td>
                        </tr>
                    </mdb-tbl-body>
                </mdb-tbl>
            ...
            <hr/>
            <mdb-card-title>Load values</mdb-card-title>
            <hr/>
            <mdb-row v-for="(carga, i) in datos.cargas" :key="'c'+i">
                <mdb-col md="2" col="md">
                    <p class="text-center align-middle my-3"><b>Load <sub>{{i+1}}</sub></b></p>
                </mdb-col>
                <mdb-col md="3" col="md">
                    <select class="browser-default custom-select my-3 disabled" v-model="carga[0]">
                            <option selected :value="undefined" disabled>Node</option>
                            <option v-for="(nodo, i) in datos.nodos" :key="'num_nodo'+i" :value="i+1">Node {{i+1}}</option>
                        </select>
                </mdb-col>
                <mdb-col md="3" col="md">
                    <select class="browser-default custom-select my-3 disabled" v-model="carga[1]">
                        ...
                        <option selected :value="3" >Moment</option>
                    </select>
                </mdb-col>
                <mdb-col md="4" col="md">
                    <mdb-input :class="carga[2].min === carga[2].max ? 'disabled' : ''" type="number" :label="'Value of load '+(i+1)+' (kN'+(carga[1] === 3 ? '·m' : '')+')'"
    ...
    <mdb-btn block v-show="editar" color="blue-grey" @click="editar = false"><mdb-icon icon="eye-slash" class="mr-1"/> Hide variable management</mdb-btn>
    <mdb-btn block v-show="!editar" color="dark-green" @click="editar = true"><mdb-icon far icon="eye" class="mr-1"/> Show variable management</mdb-btn>
    ...
    <div class="text-right">
        <mdb-btn size="sm" color="default" @click="leyenda = true"><mdb-icon icon="info-circle" class="mr-1"/> Key</mdb-btn>
    </div>
</mdb-container>
    ...
    <mdb-tabs
        ...
        { text: 'Ktot' },
        { text: 'Kred' },
        { text: 'Displacements and reactions' }]"
    ...
    >
        <template :slot="'Kloc'">
            <mdb-container>
                <mdb-card-title class="text-center">Stiffness matrices in local coordinates</mdb-card-title>
                <div v-for="(matriz, i) in localK" :key="'kloc'+i">
                    <div class="container overflow-auto">
                        <katex-element :expression="matriz" :throwOnError="false" :display-mode="true"/>
                    </div>
                    <div class="text-right">
                        <mdb-btn size="sm" color="secondary" @click="matrizSelecionada = matriz; verMatriz = true"><mdb-icon icon="search" class="mr-1"/> Zoom</mdb-btn>
                    </div>
                </div>
            </mdb-container>
        </template>
        <template :slot="'C'">
            <mdb-container>
                <mdb-card-title class="text-center">Change-of-basis matrix</mdb-card-title>
        ...
        <template :slot="'Kglob'">
            <mdb-container>
                <mdb-card-title class="text-center">Stiffness matrices in global coordinates</mdb-card-title>
        ...
        <template :slot="'Ktot'">
            <mdb-card-title class="text-center"> Full stiffness matrix of the structure before applying the boundary conditions.</mdb-card-title>
            <mdb-container>
                <div class="container overflow-auto">
                    <katex-element :expression="Ktot" :throwOnError="false" :display-mode="true"/>
                </div>
                <div class="text-right">
                    <mdb-btn size="sm" color="secondary" @click="matrizSelecionada = Ktot; verMatriz = true"><mdb-icon icon="search" class="mr-1"/> Zoom</mdb-btn>
                </div>
            </mdb-container>
        </template>
        <template :slot="'Kred'">
            <mdb-container>
                ...
                <div class="text-right">
                    <mdb-btn size="sm" color="secondary" @click="matrizSelecionada = Kred; verMatriz = true"><mdb-icon icon="search" class="mr-1"/> Zoom</mdb-btn>
                </div>
            </mdb-container>
        </template>
        <template :slot="'Displacements and reactions'">
            <div class="container overflow-auto my-3">
                <mdb-card-title class="text-center">Displacements</mdb-card-title>
                <mdb-tbl hover sm>
                    <mdb-tbl-head>
                        <tr>
                            <th>Nodes</th>
                    ...
                    <mdb-tbl-body>
                        <tr v-for="(trash, i) in datos.nodos" :key="'b'+i">
                            <td class="align-middle"><b>Node <sub>{{i+1}}</sub></b></td>
            ...
            <mdb-row class="container">
                <mdb-col size="md" md="4">
                    <mdb-input placeholder="Factor multiplicativo" class="mb-3 mt-0" v-model="factor" :min="0" :step="1" @change="verDesp = false; actualizaDibujo()">
                        <span class="input-group-text md-addon" slot="prepend">Amplifying factor</span>
                    </mdb-input>
                </mdb-col>
                <mdb-col size="md" md="8" class="text-center">
                    <mdb-btn size="sm" block v-show="verDesp" color="primary" @click="verDesp = false; actualizaDibujo()"><mdb-icon icon="eye-slash" class="mr-1"/>  Show nodal displacements</mdb-btn>
                    <mdb-btn size="sm" block v-show="!verDesp" color="default" @click="verDesp = true; actualizaDibujo()"><mdb-icon far icon="eye" class="mr-1"/> Hide nodal displacements</mdb-btn>
                </mdb-col>
            </mdb-row>
            <mdb-container class="container overflow-auto my-2">
                <mdb-card-title class="text-center">Reactions</mdb-card-title>
                <mdb-btn size="sm" block v-show="verReac" color="primary" @click="verReac = false; actualizaDibujo()"><mdb-icon icon="eye-slash" class="mr-1"/> Hide reactions</mdb-btn>
                <mdb-btn size="sm" block v-show="!verReac" color="default" @click="verReac = true; actualizaDibujo()"><mdb-icon far icon="eye" class="mr-1"/> Show reactions</mdb-btn>
