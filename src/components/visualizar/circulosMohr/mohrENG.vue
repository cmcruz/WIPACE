...
<mdb-container class="my-4">
    <blockquote class="blockquote bq-primary my-3">
        ...
        <p> Modify β angle to observe how Mohr’s circle representation and the corresponding results modify.</p>
    </blockquote>  

    <mdb-card class="my-2">
        <mdb-card-body>
            <mdb-card-title class="text-center">Computed results</mdb-card-title>
            <hr/>
            <mdb-row>
                <mdb-col md="3" col="md">
            ...
            <mdb-row>
                ...
                <mdb-col md="3" col="md">
                    Mohr C. radius : {{this.resultado.radio.toFixed(2)}} MPa
                </mdb-col>
                <mdb-col md="3" col="md">
                    Mohr C. center : {{this.resultado.centro.toFixed(2)}} MPa
                </mdb-col>
            </mdb-row>
            <mdb-row class="my-3">
                <mdb-col md="3" col="md">
            ...
            <hr/>
            <mdb-card-title class="text-center">Stresses tensor </mdb-card-title>
            <hr/>
            <katex-element :expression="formula" :throwOnError="false" :display-mode="true"/>
            <hr/>
            <mdb-card-title class="text-center">Angle &beta;</mdb-card-title>
            <div>
                <mdb-input type="number" label="Angle β" :min="0" :max="360" 
                            :step="0.1" v-model.number="datos.B" @input="cambio()"/>
                <input  type="range" :min="0" :max="360" :step="1" class="custom-range"
                        v-model="datos.B" @input="cambio()" aria-label="Angle β">
            </div>
            <hr/>
...
import dibujos from '@/components/visualizar/circulosMohr/dibujosENG';
