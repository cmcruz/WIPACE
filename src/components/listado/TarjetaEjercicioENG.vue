<template>
    <mdb-card class="hoverable mb-2">
        <mdb-view :gradient="this.obtenerColor(this.dificultad)">
            <h2 class="card-header-title text-dark text-center mt-2">Problem {{this.id}}</h2>
        ...
        <mdb-card-body class="text-center">
            ...
            <hr/>
            <router-link tag="button" class="btn btn-block elegant-color text-white" :to="{ name: 'RealizarENG', params: { tipo: this.enlace, id: this.id_bd } }" append >Solve</router-link>
            <router-link tag="button" v-show="prof" class="btn btn-block secondary-color text-white my-2" :to="{ name: 'Modificador', params: {tipo: this.enlace, id:this.id_bd}}" append >Modify</router-link>
            <mdb-btn block class="my-2" color="danger" v-show="prof" @click="borrar">Delete</mdb-btn>
        </mdb-card-body>
    </mdb-card>
</template>
